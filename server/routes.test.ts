...
describe("interdiff", () => {
  test("seen snapshots the file; interdiff returns only the delta since", async () => {
    const dir = makeRepo("routes-interdiff", {
      "a.txt": "l1\nl2\nl3\nl4\nl5\nl6\nl7\nl8\nl9\nl10\n",
    });
    git(dir, "checkout", "-b", "f");
    ...
    const clean = (await (
      await app.request(`/diff/interdiff?${q}&path=a.txt`)
    ).json()) as InterdiffResponse;
    ...
    const text = inter.file.hunks[0]!.lines.map(
      (l) => `${l.kind}:${l.text}`,
    ).join("|");
    ...
    const full = (await (
      await app.request(`/diff/file?${q}&path=a.txt`)
    ).json()) as FileDiffResponse;
    const fullText = full.file.hunks
      .flatMap((h) => h.lines)
      .map((l) => `${l.kind}:${l.text}`)
      .join("|");
  ...
  test("hash-mismatched seen stores no snapshot; deleted file diffs to all-del", async () => {
    ...
    expect((await app.request(`/diff/interdiff?${q}&path=b.txt`)).status).toBe(
      404,
    );
    ...
    const inter = (await (
      await app.request(`/diff/interdiff?${q}&path=b.txt`)
    ).json()) as InterdiffResponse;
    expect(inter.file.hunks[0]!.lines.every((l) => l.kind === "del")).toBe(
      true,
    );
