...
describe("pending / submit / ack", () => {
  test("non-pending create is submitted immediately; pending has no submittedSeq", () => {
    const now = createComment({ dir: "/r/a", base: "main", author: "agent", body: "now" });
    expect(now.status).toBe("submitted");
    expect(now.submittedSeq).not.toBeNull();
    const later = createComment({ dir: "/r/a", base: "main", author: "user", body: "later", pending: true });
  ...
  test("submission order can differ from creation order; submitted axis stays monotonic", () => {
    ...
    // would never see it.
    const a = createComment({ dir: "/r/a", base: "main", author: "user", body: "A", pending: true });
    const b = createComment({ dir: "/r/a", base: "main", author: "agent", body: "B" });
  ...
  test("multiple pending submit in creation order", () => {
    createComment({ dir: "/r/a", base: "main", author: "user", body: "1", pending: true });
    createComment({ dir: "/r/a", base: "main", author: "user", body: "2", pending: true });
    createComment({ dir: "/r/b", base: "main", author: "user", body: "other-dir", pending: true });
    expect(submitPending("/r/a").submitted).toBe(2);
    expect(listComments("/r/a", undefined, undefined, true).comments.map((c) => c.body)).toEqual(["1", "2"]);
    // other dir untouched
  ...
  test("submittedOnly list excludes pending; cursor is submitted axis", () => {
    const s = createComment({ dir: "/r/a", base: "main", author: "user", body: "sent" });
    createComment({ dir: "/r/a", base: "main", author: "user", body: "draft", pending: true });
    ...
    expect(listComments("/r/a").comments.map((c) => c.body)).toEqual(["sent", "draft"]);
  });

  test("ackPickedUp marks only submitted ≤ upTo, once", () => {
    const first = createComment({ dir: "/r/a", base: "main", author: "user", body: "first" });
    const second = createComment({ dir: "/r/a", base: "main", author: "user", body: "second" });
  ...
  test("pre-pending-schema DB migrates: columns added, submitted_seq backfilled = seq", () => {
    ...
    raw.close();
    openDb(path);
    const { comments, cursor } = listComments("/r/m", undefined, undefined, true);
    ...
    const next = createComment({ dir: "/r/m", base: "main", author: "user", body: "new" });
